...
![The domain model for our Chirp! project.](./images/domain.svg)
...
Chirp! supports GitHub integration via OAuth. Without this setup, the feature to log in with GitHub does not work when running Chirp! locally. 
...
The many end-to-end tests ensure that the entire program is running correctly and therefore also handles security tests like SQL injection, XSS, and CSRF to ensure there are no security vulnerabilities. 

# Ethics

## License
For the applications license we chose the MIT license because of its simplicity and non-restrictive nature leading to more collaboration. The MIT license is also compatible with most other licenses. To see the full license, [see our GitHub repository](https://github.com/ITU-BDSA2025-GROUP15/Chirp/blob/main/LICENSE). 
...
CodeFactor, unlike ChatGPT and Copilot, is not an LLM, but a bot that does static analysis based on rules and therefore consumes much less power than an LLM. The energy and water consumption of an LLM is hard to figure out, due to the companies not releasing official numbers. The amount is likely high and is the reason the group only used the LLMs to a limited degree. 
